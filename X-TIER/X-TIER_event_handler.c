...
static int XTIER_event_handler_virustotal(CPUState *env, u32 a, u32 b, u32 c, u32 d, u32 e)
{
	char cmd[2048];

	sprintf(cmd, "/tmp/scripts/virustotal/virustotal.py %08x%08x%08x%08x%08x", a, b, c, d, e);
...
/**
 * post receive hook for a printk message. used for invoking e.g. virus scans.
 */
                                       ...
                                       struct XTIER_x86_call_registers *call_regs)
{
	...
	if (0 == strncmp(msg, "SHA", 3)) {
		// This is a virus total command
		vm_regs->rax = XTIER_event_handler_virustotal(state,
		                                              (u32)call_regs->rsi,
		                                              (u32)call_regs->rdx,
		                                              (u32)call_regs->rcx,
		                                              (u32)call_regs->r8,
		...
		vm_regs->rax = XTIER_event_handler_clamav_simple(state,
		                                                 (u32)call_regs->rsi,
		                                                 (u32)call_regs->rdx,
		                                                 (u32)call_regs->rcx,
		                                                 (u32)call_regs->r8,
